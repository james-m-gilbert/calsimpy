...
class calsimhydro(object):
    ...
    def __init__(self,projDir=''):
        ...
        self.RFRO_StartDate = None
        self.RFRO_EndDate = None
        self.RFRO_PrecipFP = None
        self.RFRO_OutMonPrecipFP = None
        self.RFRO_LandUseDict = None
        self.RFRO_WBAList = None
        self.RFRO_PrecipData = {}
...
#                desc2 = [desc]
        ...
        self.RFRO_PrecipFP = precip_fp
        self.RFRO_OutMonPrecipFP = outMonPrecip_fp
        self.RFRO_LandUseDict = lu_dict
        self.RFRO_WBAList = wba_list
        self.RFRO_StartDate = beginDT
        self.RFRO_EndDate = endDT
        
        return([scenName, lu_by_soil_fp, param_fp, precip_fp, outMonPrecip_fp,
                NREGN, NLUCodes, lu_dict, wba_list])


    def get_precip_paths(self):
        
        pcp_fp = self.RFRO_PrecipFP
    
        with open(os.path.join(self.projDir, pcp_fp),'r') as of:
            rl = of.readlines()
        ...
        for r in range(1, len(rlnc)):
            ...
            cpath = rlnc[r].split()[0].strip()
            dum1, ap,bp,cp,dp,ep,fp,dum2 = cpath.split("/")
            iregn = bp
            pcp_path_dict[iregn] = cpath
        
        return([DSSFL,fullDSSFL, pcp_path_dict]) #, etfact

        
    def get_daily_precip(self, precip_startDate = None, precip_endDate = None, 
                         ctime24=True, target_units = 'default'):
        
        # target unit options: 'IN', 'MM' (inches, millimeters)
        # 'default'  will use whatever original data are
    
        [DSSFL, fullDSSFL, path_dict] = self.get_precip_paths()

        if precip_startDate==None:
            precip_startDate = self.RFRO_StartDate
        if precip_endDate ==None:
            precip_endDate = self.RFRO_EndDate

        if len(path_dict)>100:
            print("lookout...just about to read in a bunch (%s) of DSS time series..." %len(path_dict))

        pcpDSS = util.dssFile(fp=fullDSSFL)

        i = 0
        
        for iregn in path_dict: # iterate through all demand unit/land use type combinations

            
            cpath = path_dict[iregn]
            
            i +=1
            # provide a progress bar in the console
            util.progress(i, len(path_dict), status='Retrieving %s Precipitation Time Series' %len(path_dict))
            
            thisvar = util.dssVar(pcpDSS, cpath=cpath)
            
            thisvar.getRTS(precip_startDate,ctime24=True, endDateTime=precip_endDate)
            
            thisPrecip= thisvar.RTS
            
            if thisvar.Units.upper() in ['IN/DAY','IN', 'INCH', 'INCHES']:
                if target_units.upper() in ['MM','MM/DAY','MILLIMETERS','MILLIMETER']:
                    thisPrecip = thisPrecip*25.4
                    if '/DAY' in thisvar.Units.upper():
                        newUnits = 'MM/DAY'
                    else:
                        newUnits = 'MM'
                else:
                    thisPrecip = thisPrecip

                    newUnits = thisvar.Units.upper()
            
            elif thisvar.Units.upper() in ['MM/DAY','MM', 'MILLIMETER','MILLIMETERS']:
                 if target_units.upper() in ['IN/DAY','IN', 'INCH', 'INCHES']:
                     thisPrecip = thisPrecip/25.4
                     if '/DAY' in thisvar.Units.upper():
                         newUnits = 'IN/DAY'
                     else:
            ...
            self.RFRO_PrecipData[iregn] = [thisPrecip, newUnits, thisvar.RecordType]
            
        pcpDSS.closeDSS()   
        
    def precip_ts_to_df(self, ptA = 'IWFM',ptC='PRECIP', ptF='PRECIPITATION'):
        
        # assumes you've retrieved the precip time series from DSS already
        
        if len(self.RFRO_PrecipData) ==0:
            print("\n\t\t**** No precip time series available - try `get_daily_precip` function")
            return()
        else:
            ...
            allregns = list(self.RFRO_PrecipData.keys())
            dtList = self.RFRO_PrecipData[allregns[0]][0].index #get dtlist from example data index
            
            basedf = pnd.DataFrame(index=dtList, columns = col_mindex )
            
            listDF = []

            
            for iregn in self.RFRO_PrecipData: # iterate through keys of data dictionary
                tmpDF = basedf.copy()
                thisrectype = self.RFRO_PrecipData[iregn][2]
                thisunits = self.RFRO_PrecipData[iregn][1]
                thisdat = self.RFRO_PrecipData[iregn][0]
                tmpDF[ptA, iregn, ptC, '1DAY', ptF, thisrectype,
                             thisunits] = thisdat
                listDF.append(tmpDF)
            
            final_df = pnd.concat(listDF, axis=1)
...
class calsim():
    ...
    def __init__(self, **kwargs): #launchFP):
        ...
        if 'launchFP' in kwargs:
            ...
            self.ConfigFP = kwargs['configFP']
            
            self.projDir = os.path.dirname(self.ConfigFP)
            if self.Reorg:
                self.TableDir = os.path.join(self.projDir, 'Run', 'Lookup')
            else:
                self.TableDir = os.path.join(self.projDir, 'CONV', 'Run', 'Lookup')
            
            self.LaunchDict = self.initLaunchDict()
            
            with open(self.ConfigFP, 'r') as cf:
                rl = cf.readlines()
            ...
            for r in rl:
                if r[0]!='#' and r[0]!='\n':
                    config_dat.append(r)
                    rs = r.split()
                    config_dict[rs[0]] = rs[1:]
                    
            # ????
            # Note: would have been nice to name variables consistently so we
            #      could do something like looping over the config dictionary
            #     and assign directly to the object with teh same key along the 
            #     lines of for k,v in config_dict.items(): setattr(self, k, v)
            #     alas....something for another day....
            self.SV_FP = config_dict['SvarFile'][0]
            self.SvarAPart = config_dict['SvarAPart'][0]
            self.SvarFPart = config_dict['SvarFPart'][0]
            self.F_Part = self.SvarFPart
            self.DvarAPart = self.SvarAPart
            self.DV_FP = config_dict['DvarFile'][0]
            self.MainFile = config_dict['MainFile'][0]
            self.InitFile = config_dict['InitFile'][0]
            self.InitFPart = config_dict['InitFPart'][0]
            self.DvarFile = self.DV_FP
            self.SvarFile = self.SV_FP
            self.GroundwaterDir = config_dict['GroundwaterDir'][0]
            self.WreslPlus = config_dict['WreslPlus'][0]
            
            self.TimeStep = config_dict['TimeStep'][0]
            self.StartDay = int(config_dict['StartDay'][0])
            self.StartMonth = int(config_dict['StartMonth'][0])
            self.StartYear = int(config_dict['StartYear'][0])
            self.StartDate = dt.datetime(self.StartYear, self.StartMonth, self.StartDay,0,0)
            self.EndDay = int(config_dict['StopDay'][0])
            self.EndMonth = int(config_dict['StopMonth'][0])
            self.EndYear = int(config_dict['StopYear'][0])
            self.EndDate = dt.datetime(self.EndYear, self.EndMonth, self.EndDay)
            self.StopYear = self.EndYear 
            self.StopMonth = self.EndMonth 
            self.StopDay = self.EndDay         
...
#                    with open(self.LaunchFP,'w')
    ...
    def getLaunchDict(self):
        if os.path.exists(self.LaunchFP):
            ...
            raise FileNotFoundError(f"Launch file {self.LaunchFP} was not found")
            return(None)
    ...
    def getIOinfo(self):
        ...
        try:
            ...
            try:
                self.DV_FP = (Path(self.projDir) / self.LaunchDict['DVAR']).resolve()
                self.SV_FP = (Path(self.projDir) / self.LaunchDict['SVAR']).resolve()
            except:
                raise FileNotFoundError(f"Couldn't assign full path to SVAR and/or DVAR files\n DVAR: {self.LaunchDict['DVAR']}")
        ...
        self.SvarAPart = self.LaunchDict['APART'] #CALSIM'
        ...
        if not os.path.exists(self.DV_FP):
            if self.Reorg:
                try:
                    if  self.LaunchDict['DVAR'].upper()[0:3]=='DSS': #no leading slash
                        oldpath, relpath = self.LaunchDict['DVAR'].upper().split('DSS\\')
                    else:
                        oldpath, relpath = self.LaunchDict['DVAR'].upper().split('\\DSS\\')
                except:
                    raise FileNotFoundError(f"Couldn't find DV Filepath {self.LaunchDict['DVAR'].upper()}")
            else:
...
#                relpath = relpath[3:]
        ...
        if not os.path.exists(self.SV_FP):
            if self.Reorg:
                if  self.LaunchDict['SVAR'].upper()[0:3]=='DSS': #no leading slash
...
#                relpath = relpath[3:]
            ...
            if oldpath=='': # relative path provided in launch file
                if self.LaunchDict['SVAR'].upper()[0:3]=='DSS': #no leading slash
                    self.SV_FP = self.projDir + '\\DSS\\' + self.LaunchDict['SVAR'].split('DSS\\')[-1]
                else:
    ...
    def config_to_launchDict(self, csvers=3):
        
        self.initLaunchDict(csversion=csvers)
        
        self.LaunchDict['SVAR'] = self.SV_FP
        self.LaunchDict['DVAR'] = self.DV_FP
        self.LaunchDict['INIT'] = self.InitFile
        self.LaunchDict['INITFPART'] = self.InitFPart
        self.LaunchDict['APART'] = self.SvarAPart
        self.LaunchDict['SVFPART'] = self.F_Part
        self.LaunchDict['TIMESTEP'] = self.TimeStep 
        self.LaunchDict['STARTDAY'] = self.StartDay
        self.LaunchDict['STARTMONTH'] = dt.date.strftime(dt.date(2020, self.StartMonth,15),'%b').lower()
        self.LaunchDict['STARTYEAR'] = self.StartYear
        self.LaunchDict['ENDDAY'] = self.EndDay
        self.LaunchDict['ENDMONTH'] = dt.date.strftime(dt.date(2020, self.EndMonth,15),'%b').lower()
        self.LaunchDict['ENDYEAR'] = self.EndYear
        
        self.LaunchDict['PROGRAM'] = self.MainFile
        self.LaunchDict['GWDATA'] = self.GroundwaterDir
        self.LaunchDict['WRESLPLUS'] = self.WreslPlus
    ...
    def write_launch_file(self, new_launch_fp,**kwargs):
        ...
        if 'Description' in kwargs:
            self.LaunchDict['DESCRIPTION'] = kwargs['Description']
    ...
    def write_config(self, **kwargs):
        ...
        if not os.path.isabs(self.DvarFile):
            self.DvarFile = os.path.join(self.projDir, self.DvarFile)
        if not os.path.isabs(self.SvarFile):
            self.SvarFile = os.path.join(self.projDir, self.SvarFile)
        if not os.path.isabs(self.InitFile):
            self.InitFile = os.path.join(self.projDir, self.InitFile)
    ...
    def copy_study(self,dest_dir, clean_SV=True, clean_DV=True, clean_INIT=False,
                   git_fork=False, overwrite=False):
        import shutil, errno
        import glob

        def copyanything(src, dst, overwrite=overwrite):
            try:
                if overwrite:
        ...
        if self.Reorg:
            ...
            copyanything(src, dst, overwrite=overwrite)
            ...
            if clean_DV:
                try:
                    ...
                    try:
                        os.remove(os.path.join(dest_dir,'DSS',os.path.basename(self.DV_FP)))
                    except:
                        print(f"Could not find file {self.DV_FP} to delete it..")
                for f in glob.glob(os.path.join(dest_dir, 'DSS', 'output', '*.ds*')):
                    os.remove(f)
                if os.path.exists(os.path.join(dest_dir, 'DSS','output','GWhyd.out')):
            ...
            # remove extraneous *.dsc, *.dsd, *.dsk files if possible
            for f in glob.glob(os.path.join(dest_dir, 'DSS', '*.dsk')):
                os.remove(f)
            for f in glob.glob(os.path.join(dest_dir, 'DSS',  '*.dsc')):
                os.remove(f)
            for f in glob.glob(os.path.join(dest_dir, 'DSS', '*.dsd')):
                os.remove(f)
                
        if git_fork:
            gitpath = os.path.join(self.projDir, '.git')
            if os.path.exists(gitpath):
                destgit = os.path.join(dest_dir, '.git')
                copyanything(gitpath, destgit)
            if os.path.exists(os.path.join(self.projDir, '.gitignore')):
                destgitig = os.path.join(dest_dir, '.gitignore')
                copyanything(os.path.join(self.projDir, '.gitignore'), destgitig)
...
class csDVdata(calsim):
    ...
    def getDVts(self, **kwargs):
        ...
        # 20231207 jmg-ucsc: attempting to solve Dino's catalog read issue
        #                    it looks like there may be a chance the DSS file
        #                    is not being initialized properly on repeated calls
        #if self.DV_DSS == None:        
        ...
        if self.DV_DSS.PathList == None:
            self.DV_DSS.get_cat_paths()  # this reads the catalog path list into the DV DSS object

        if self.DV_DSS.PathList ==[]:
            print('\n\n******************************************')
            print('\tEmpty Catalog Path List - Trying Again')
            print('\t to retrieve the catalog data')
            print(f'\t {self.csObj.DV_FP}')      
            print('*******************************************')
            self.DV_DSS.get_cat_paths()  # this reads the catalog path list into the DV DSS object
        
        if self.DV_DSS.CndPathList==None:
...
class ensembleCS(calsim):  
    ...
    def prepare_years(self, skip_tables=False, prior_pad=3):
        ...
        years = range(af.addWY(self.csObj.StartDate)-prior_pad,af.addWY(self.csObj.EndDate)+2)
        ...
        for iy, ns in enumerate(sv_years_span): #enumerate(np.arange(1921, 2003)):
            ...
            for en in tmpexclude:
                if (en == tmpyr+1) & (en in self.csObj.Years): #thisYr+1:   # this is an attempt to retain the years excluded from shuffling in the right order
                    ...
                    #reordered_years[en] = nyr
                    if en not in reordered_years[-5:]:
                        reordered_years.append(en)
                        reordered_yr_data.append(nyr)
                        new_shuff.append(en)
                        sequential_yrs.append(en)
